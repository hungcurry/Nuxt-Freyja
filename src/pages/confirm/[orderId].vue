...
<template>
  <div class="overflow-hidden pt-18 pt-md-30 bg-neutral-120">
        ...
        <div class="col-12 col-md-6">
          ...
          >
            <div>
              ...
              <span class="text-neutral-0 fw-bold">{{ bookingResult?.userInfo?.name }}</span>
            </div>
            <div>
              <p class="mb-2 text-neutral-40 fw-medium">
                手機號碼
              </p>
              <span class="text-neutral-0 fw-bold">{{ bookingResult?.userInfo?.phone }}</span>
            </div>
            <div>
              <p class="mb-2 text-neutral-40 fw-medium">
                電子信箱
              </p>
              <span class="text-neutral-0 fw-bold">{{ bookingResult?.userInfo?.email }}</span>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <section class="d-flex flex-column gap-6">
              ...
              <div class="text-neutral-80 fs-8 fs-md-7 fw-bold">
                <p class="title-deco mb-2">
                  入住：{{ formatDateWeekday(bookingInfo?.checkInDate || '') }}，15:00 可入住
                </p>
                <p
                  class="title-deco mb-0"
                >
                  退房：{{ formatDateWeekday(bookingInfo?.checkOutDate || '') }}，12:00 前退房
                </p>
              </div>

              <p class="mb-0 text-neutral-80 fs-8 fs-md-7 fw-bold">
                NT$ {{ bookingResult?.totalPrice }}
              </p>
            </section>
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 fs-8 fs-md-7 bg-neutral-0 border border-neutral-40 rounded-3 list-unstyled">
                <li
                  v-for="room in bookingResult?.facilityInfo"
                  :key="room.title"
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 fs-8 fs-md-7 bg-neutral-0 border border-neutral-40 rounded-3 list-unstyled">
                <li
                  v-for="item in bookingResult?.amenityInfo"
                  :key="item.title"
                  class="flex-item d-flex gap-2"
                >
                  <Icon
